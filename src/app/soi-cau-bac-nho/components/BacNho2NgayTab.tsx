...
export default function BacNho2NgayTab({ toDate }: { toDate: string }) {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/soi-cau-bac-nho/2-ngay?days=100${toDate ? `&toDate=${toDate}` : ''}`);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 text-xs text-blue-700 flex items-center gap-2">
                <span>ℹ️</span>
                <span>Baseline ngẫu nhiên ≈ <strong>27%</strong>. Tab 2 Ngày: A xuất hiện hôm kia, B xuất hiện hôm qua → dự đoán C hôm nay.</span>
            </div>
                ...
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border-2 border-indigo-300 p-5 rounded-xl">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-indigo-600">Số được nhiều cặp AB 2-ngày độc lập cùng dự đoán</p>
                        </div>
                    </div>
            ...
            {displayedPredictions.length > 0 && (
                <div className="bg-gradient-to-br from-yellow-50 to-orange-50 border-l-4 border-orange-500 p-6 rounded-lg">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                        <h2 className="text-xl font-bold text-orange-700">🔥 Dự Đoán Hôm Nay (Hôm Kia + Hôm Qua)</h2>
                                ...
                                <div key={pairToKey(yesterdayPair)} className={`bg-white rounded-lg p-4 shadow-md border-2 ${viewMode === 'percentage' ? 'border-orange-200 hover:border-orange-400' : 'border-purple-200 hover:border-purple-400'}`}>
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="text-sm text-gray-600">
                                            Hôm kia <span className={`font-bold ${viewMode === 'percentage' ? 'text-orange-600' : 'text-purple-600'}`}>{yesterdayPair[0]}</span>,
                                            hôm qua <span className={`font-bold ${viewMode === 'percentage' ? 'text-orange-600' : 'text-purple-600'}`}>{yesterdayPair[1]}</span>
                                        </div>
            ...
            <div>
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                    <h2 className="text-xl font-bold text-lottery-gray-800">Tất Cả Patterns 2 Ngày</h2>
                                ...
                                return (
                                    <React.Fragment key={key}>
                                        <tr className={`border-b border-lottery-gray-200 hover:bg-lottery-gray-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-lottery-gray-50'}`} onClick={() => toggleRow(key)}>
                                            <td className="px-4 py-3"><span className="stats-number text-lg font-bold">{pattern.triggerPair[0]} → {pattern.triggerPair[1]}</span></td>
