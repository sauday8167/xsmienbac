...
export default function Cap2Tab({ toDate }: { toDate: string }) {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/soi-cau-bac-nho/cap-2?days=100${toDate ? `&toDate=${toDate}` : ''}`);
    ...
    // Consensus: count which numbers are predicted by most trigger pairs
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 text-xs text-blue-700 flex items-center gap-2">
                <span>ℹ️</span>
                <span>Baseline ngẫu nhiên XSMB ≈ <strong>27%</strong>. Chỉ có giá trị tham khảo khi tỷ lệ ≥ 40% và cặp xuất hiện ≥ 5 lần.</span>
            </div>
                ...
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border-2 border-indigo-300 p-5 rounded-xl">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-indigo-600">Số được nhiều cặp trigger độc lập cùng dự đoán — đáng tin hơn từng cặp riêng lẻ</p>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-3">
                        {consensusNumbers.map((item, idx) => (
                            <div key={item.number} className={`flex flex-col items-center p-3 rounded-xl border-2 min-w-[68px] ${
                                ...
                                <div className={`text-2xl font-black ${idx === 0 ? 'text-yellow-700' : idx === 1 ? 'text-gray-600' : idx === 2 ? 'text-orange-700' : 'text-indigo-700'}`}>
                                    {item.number}
                                </div>
            ...
            {displayedPredictions.length > 0 && (
                <div className="bg-gradient-to-br from-yellow-50 to-orange-50 border-l-4 border-orange-500 p-6 rounded-lg">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                        <h2 className="text-xl font-bold text-orange-700">🔥 Dự Đoán Hôm Nay (Dựa Vào Cặp Hôm Qua)</h2>
                                ...
                                <div key={pairToKey(yesterdayPair)} className={`bg-white rounded-lg p-4 shadow-md border-2 ${viewMode === 'percentage' ? 'border-orange-200 hover:border-orange-400' : 'border-purple-200 hover:border-purple-400'}`}>
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="text-sm text-gray-600">
                                            Hôm qua cặp: <span className={`font-bold text-lg ${viewMode === 'percentage' ? 'text-orange-600' : 'text-purple-600'}`}>{yesterdayPair[0]} + {yesterdayPair[1]}</span>
                                        </div>
            ...
            <div>
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                    <h2 className="text-xl font-bold text-lottery-gray-800">Tất Cả Patterns Cặp 2</h2>
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {paginatedPatterns.map((pattern, index) => {
                                const key = pairToKey(pattern.triggerPair);
                                        ...
                                        {expandedRows.has(key) && (
                                            <tr className="bg-green-50">
                                                <td colSpan={4} className="px-4 py-4">
                                                    <strong className="text-green-700 text-sm">Tất cả số C theo tỷ lệ:</strong>
