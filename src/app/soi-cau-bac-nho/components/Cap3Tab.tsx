...
export default function Cap3Tab({ toDate }: { toDate: string }) {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/soi-cau-bac-nho/cap-3?days=100${toDate ? `&toDate=${toDate}` : ''}`);
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 border-l-4 border-purple-600 p-4 rounded-lg">
                    <div className="text-sm text-lottery-gray-600 mb-1">Tổng bộ 3 patterns</div>
                    <div className="text-2xl font-bold text-purple-600">{filteredPatterns.length}</div>
                </div>
            </div>

            <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 text-xs text-blue-700 flex items-center gap-2">
                <span>ℹ️</span>
                <span>Baseline ngẫu nhiên XSMB ≈ <strong>27%</strong>. Bộ 3 có ít mẫu hơn Cặp 2, cần ≥ 3 lần xuất hiện để có ý nghĩa.</span>
            </div>
                ...
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border-2 border-indigo-300 p-5 rounded-xl">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-indigo-600">Số được nhiều bộ 3 trigger độc lập cùng dự đoán</p>
                        </div>
                    </div>
            ...
            {displayedPredictions.length > 0 && (
                <div className="bg-gradient-to-br from-yellow-50 to-orange-50 border-l-4 border-orange-500 p-6 rounded-lg">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                        <h2 className="text-xl font-bold text-orange-700">🔥 Dự Đoán Hôm Nay (Dựa Vào Bộ 3 Hôm Qua)</h2>
                                ...
                                <div key={tripleToKey(yesterdayTriple)} className={`bg-white rounded-lg p-4 shadow-md border-2 ${viewMode === 'percentage' ? 'border-orange-200 hover:border-orange-400' : 'border-purple-200 hover:border-purple-400'}`}>
                                    <div className="flex items-start justify-between mb-2 gap-2">
                                        <div className="text-sm text-gray-600">
                                            Hôm qua bộ 3: <span className={`font-bold ${viewMode === 'percentage' ? 'text-orange-600' : 'text-purple-600'}`}>{yesterdayTriple[0]}+{yesterdayTriple[1]}+{yesterdayTriple[2]}</span>
                                        </div>
            ...
            <div>
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                    <h2 className="text-xl font-bold text-lottery-gray-800">Patterns Bộ 3</h2>
                ...
                <div className="hidden md:block overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-lottery-gray-100">
                            <tr>
                                <th className="px-4 py-3 text-left text-sm font-semibold">Bộ 3 A+B+C (Trigger)</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold">Lần xuất hiện</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold">Top số D dự đoán</th>
                                        ...
                                        {expandedRows.has(key) && (
                                            <tr className="bg-green-50">
                                                <td colSpan={4} className="px-4 py-4">
                                                    <strong className="text-green-700 text-sm">Tất cả số D theo tỷ lệ:</strong>
