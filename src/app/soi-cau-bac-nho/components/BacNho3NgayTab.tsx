...
export default function BacNho3NgayTab({ }: Props) {
        ...
        <div className="space-y-6">
                ...
                <div className="bg-gradient-to-br from-yellow-50 to-orange-50 border-l-4 border-orange-500 p-6 rounded-lg">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                        <h2 className="text-2xl font-bold text-orange-700">
                            🔥 Dự Đoán Hôm Nay (Dựa Vào 3 Ngày)
                        </h2>
                        ...
                        {displayedPredictions.slice(0, 9).map(({ triggerTriple, predictions }) => (
                            ...
                            >
                                <div className="text-sm text-gray-600 mb-2">
                                    Ngày trước kia{' '}
                                    <span className={`font-bold text-base ${viewMode === 'percentage' ? 'text-orange-600' : 'text-purple-600'}`}>
                                        {triggerTriple[0]}
                                    </span>
                                    , hôm kia{' '}
                                    <span className={`font-bold text-base ${viewMode === 'percentage' ? 'text-orange-600' : 'text-purple-600'}`}>
                    ...
                    {displayedPredictions.length === 0 && (
                        <div className="text-center text-gray-500 py-8">
                            ...
                            <div>Không tìm thấy bộ 3 ngày khớp với lịch sử. Hãy thử giảm bộ lọc tỷ lệ.</div>
                        </div>
                    )}
                </div>
            )}
            ...
            <div>
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                    <h2 className="text-2xl font-bold text-lottery-gray-800">Tất Cả Patterns 3 Ngày</h2>
                    <select
                        value={filterRate}
                                ...
                                return (
                                    <React.Fragment key={tripleKey}>
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 font-semibold">{pattern.totalAppearances} lần</td>
                                            <td className="px-4 py-3">
                            ...
                            >
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="min-w-[5rem] h-12 px-3 rounded-full bg-lottery-red-600 flex items-center justify-center text-white text-sm font-bold shadow-sm whitespace-nowrap">
                                            {pattern.triggerTriple[0]}&nbsp;&amp;&nbsp;{pattern.triggerTriple[1]}&nbsp;&amp;&nbsp;{pattern.triggerTriple[2]}
                                        </div>
                                        <div>
                                            <div className="text-xs text-lottery-gray-500 font-semibold uppercase">Bộ trigger 3 ngày</div>
                                            <div className="font-bold text-lottery-gray-800">{pattern.totalAppearances} lần xuất hiện</div>
                                        </div>
                                    </div>
