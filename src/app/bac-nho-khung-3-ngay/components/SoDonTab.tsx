...
import type { BacNhoSoDonData } from '@/types/bac-nho-types';
...
export default function SoDonTab({ }: Props) {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/bac-nho-khung-3-ngay/so-don?days=${selectedDays}`);
            ...
            console.error('Error fetching Bạc Nhớ Số Đơn Khung 3 Ngày:', error);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="bg-gradient-to-br from-yellow-50 to-orange-50 border-l-4 border-orange-500 p-6 rounded-lg">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                        <h2 className="text-2xl font-bold text-orange-700">
                            🔥 Gợi Ý Nuôi Khung 3 Ngày (Từ hôm nay)
                        </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {displayedPredictions.slice(0, 9).map(({ yesterdayNumber, predictions }) => (
                            <div
                                key={yesterdayNumber}
                            ...
                            >
                                <div className="text-sm text-gray-600 mb-2">
                                    Về số: <span className={`font-bold text-lg ${viewMode === 'percentage' ? 'text-orange-600' : 'text-purple-600'
                                        }`}>{yesterdayNumber}</span>
                                </div>
                                    ...
                                    predictions.slice(0, 3).map((pred, idx) => (
                                        <div key={pred.number} className="mb-2">
                                            <div className="text-base font-semibold text-green-700">
                                                ...
                                                {idx === 1 && '➋ '}
                                                {idx === 2 && '➌ '}
                                                Nuôi {pred.number} - {pred.correlationRate.toFixed(1)}%
                                            </div>
                                    ...
                                    predictions.slice(0, 3).map((pred, idx) => (
                                        <div key={pred.number} className="mb-2">
                                            <div className="text-base font-semibold text-blue-700">
                                                ...
                                                {idx === 1 && '🥈 '}
                                                {idx === 2 && '🥉 '}
                                                Nuôi {pred.number} - {pred.hitCount} lần
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <div className="flex-1 bg-gray-200 rounded-full h-2">
                                                    <div
                ...
                <div className="hidden md:block overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-lottery-gray-100">
                            <tr>
                                <th className="px-4 py-3 text-left text-sm font-semibold">Số Về (Trigger)</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold">Lần xuất hiện</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold">Số Nuôi Khung (3 ngày)</th>
                                        ...
                                        <tr className="bg-green-50">
                                            <td colSpan={4} className="px-4 py-4">
                                                <div className="text-sm">
                                                    <strong className="text-green-700">Tất cả số nuôi theo tỷ lệ &gt; {filterRate}%:</strong>
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div className="text-xs text-lottery-gray-500 font-semibold uppercase">Số vừa về</div>
                            ...
                            <div className="p-4 border-t border-lottery-gray-100">
                                <div className="text-xs font-bold text-green-700 uppercase mb-3">Top số nuôi (Khung 3 ngày):</div>
                            ...
                            {expandedRows.has(pattern.triggerNumber) && (
                                <div className="p-4 bg-green-50/50 border-t border-green-100 animate-fade-in">
                                    <div className="text-xs font-bold text-green-700 uppercase mb-3">Tất cả các số nuôi liên quan:</div>
