...
export default function RandomDrawPage() {
    ...
    // Fetch AI Predictions to match "Winning" numbers
    useEffect(() => {
        const fetchPredictions = async () => {
            try {
                const response = await fetch('/api/admin/ai/run-prediction'); // Or a public API if available
                const data = await response.json();
                if (data.success && data.data) {
                    setWinningNumbers(data.data.dan_loto || []);
                }
            } catch (error) {
        ...
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2">
                    <TrialBoard results={results} statusArray={statusArray} winningNumbers={winningNumbers} />
                </div>
