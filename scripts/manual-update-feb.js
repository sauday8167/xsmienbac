...
const { query, queryOne, closePool } = require('../src/lib/db');
...
async function crawlAndSave(date) {
    ...
    try {
        ...
        // Backup for Special/G1 if ID failed (sometimes they change IDs)
        ...
        if (!result.special_prize) {
            ...
            // Try one more fallback for Prize Special if it's empty - sometimes it's just .special-prize in a table
