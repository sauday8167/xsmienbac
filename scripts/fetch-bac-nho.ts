...
async function fetchBacNho() {
    ...
    // If run after 18:30, it's predicting for TOMORROW
    if (h > 18 || (h === 18 && m >= 30)) {
        now.setDate(now.getDate() + 1);
    }
    const targetDateVN = now.toLocaleDateString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' });
    ...
    await query(`
        ...
        ON CONFLICT(draw_date) DO UPDATE SET
            ...
            hit_numbers = NULL,
            hit_count = 0
    `, [targetDateVN, JSON.stringify(top10), JSON.stringify(scoreBreakdown), analysisContent]);
    
    console.log(`✅ Đã lưu dữ liệu Hội Đồng Bạc Nhớ Tổng Hợp cho ngày ${targetDateVN}`);
    process.exit(0);
}
