...
async function crawlAndSave(date) {
    ...
    try {
        ...
        const getVal = (id) => $(`#${id}`).text().trim();
        const getList = (baseId, count) => {
            const arr = [];
            for (let i = 0; i < count; i++) {
                // Try 0-based and 1-based just in case
                let val = getVal(`${baseId}_item${i}`);
                if (!val) val = getVal(`${baseId}_item${i + 1}`);
        ...
        // Arrays
        // Note: xoso.com.vn IDs usually: mb_prize2_item0, mb_prize2_item1 for G2.
        // Let's rely on class if IDs fail, but IDs are best.

        // Helper to find by partial ID if needed? No, let's use wildcards if specific counts fail.
        // Actually, let's use a smarter selector: [id^="mb_prize2_item"]
        ...
        const getByPrefix = (prefix) => {
            ...
            return arr.sort(); // Sorting might be wrong order, but usually document order is correct.
            // Actually, we should trust document order.
            // .map return array
        ...
        if (!result.special_prize) {
            console.error('❌ Could not find Special Prize. Aborting save.');
